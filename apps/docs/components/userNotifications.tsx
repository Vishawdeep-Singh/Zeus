...
import { notificationsState, unreadNotificationsSelector, warningNotificationsState } from "@/states/notifications";
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <div
          className="relative h-14 w-14 hover:border-2 p-[2px] rounded-full border-black hover:cursor-pointer"
        >
       ...
       {warningNotifications.length>0 && <Badge
         
         
         className="absolute p-1 rounded-full bg-yellow-500 -top-2 -left-2 h-5 w-5 flex items-center justify-center text-xs"
       >
          ...
          {unreadNotifications.length > 0 || warningNotifications.length>0 ? (
            <ScrollArea className="h-[300px] pr-4">
              <ol className="space-y-4">
               {warningNotifications.length>0 && warningNotifications.map((warningNotification)=>{
                return <li key={warningNotification.id} className="flex space-x-4">
