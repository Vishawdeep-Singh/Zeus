...
export async function MembersofGym({membersDetails}:any){
  console.log("Inside members of gym",membersDetails)
    return  <div className=" h-96 w-full mx-auto overflow-y-auto bg-muted bg-opacity-100 shadow-sm  hover:shadow-2xl hover:shadow-primary/30  hover:scale-105 transition-all  duration-300 hover:bg-white rounded-xl border p-4">
            ...
            {membersDetails.map((member:any,i:any)=>{
              ...
              let JoinedDate=moment(member.memberships?.[0].dateJoined ).format('DD/MM/YYYY')
    ...
    console.log(differenceInDays)
          ...
          <div className="h-12 w-12 mr-3 rounded-full flex-shrink-0 bg-black">
...
<div className="flex flex-col space-y-4">
  ...
  <div className="flex justify-between items-center">
    ...
    {/* Icon section */}
    <div>
      <Zap fill={member.memberships?.[0].membership.color} className="h-6 w-6"></Zap>
    </div>
  </div>
  ...
  <div className="text-gray-600 text-sm">
    ...
    <div><strong>Phone:</strong> {member.cellPh}</div>
    <div><strong>Joined:</strong> {JoinedDate}</div>
    <div><strong>Days Left:</strong> {differenceInDays}</div>
    <div><strong>Membership:</strong> {member.memberships?.[0].membership.duration} months , Rs. {member.memberships?.[0].membership.price}</div>
  </div>
</div>
</PopoverContent>
