...
export const FileUpload = ({
  ...
  useEffect(() => {
    return () => {
      setFile(null); // Clear file when component unmounts
  ...
  const handleFileChange = (newFile: File) => {
    ...
    if (newFile.size / (1024 * 1024) > MAX_FILE_SIZE_MB) {
  ...
  const { getRootProps, isDragActive } = useDropzone({
    ...
    onDrop: (acceptedFiles) => {
      if (acceptedFiles.length > 0) {
        handleFileChange(acceptedFiles[0] as File); // Handle only the first file
      }
    },
        ...
        <input
          ...
          onChange={(e) => {
            if (e.target.files && e.target.files[0]) {
              handleFileChange(e.target.files[0]); // Set only the first file
            }
          }}
          className="hidden"
        />
